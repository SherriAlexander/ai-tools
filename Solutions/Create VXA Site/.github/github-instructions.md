...
- **At the close of any session** involving script execution, terminal commands, API calls, or MCP tool use, run the `session-retrospective` skill. Do not wait to be asked — treat this as a mandatory session-close step. The skill will propose doc updates for confirmation before writing anything.
