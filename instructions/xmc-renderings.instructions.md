...
One important translation rule applies: if the JIRA ticket lists the rendering group as **"Content"**, it must be written as **"VXA Body Content"** — not "VXA Content". This is a known inconsistency between ticket language and system naming that should always be corrected silently during script generation.
...
The rendering name must always be prefixed with "VXA ". For example, if the ticket title is "Summary", the rendering name should be set to "VXA Summary". If the ticket title is "Pull Quote", the rendering name should be "VXA Pull Quote".

### Example : rendering name definition

```powershell
$renderingName = "VXA <rendering-name-from-ticket>"
```

## ICON : REQUIRED

When creating a Sitecore datasource template and rendering item, an icon can be set to help users easily identify it within the Sitecore content management system. The icon should be a powershell variable that will be used when creating the rendering definition. The icon value should be located in the ticket under 'Authoring Considerations'.

There are two types of icon references:

**1. System icon paths** — paths that reference built-in Sitecore icons. These start with `/~/icon/` or have no leading path segment:
...
For these, convert to a relative path by stripping the leading `/~/icon/`. For example, `/~/icon/office/32x32/books.png` becomes `office/32x32/books.png`.

**2. Media library paths** — paths that reference an uploaded image in the Sitecore media library. These start with `/-/media/`:

- /-/media/6c8f87cd976e4fd6b9813c121037651a.ashx?thn=1&&db=master

For these, use the path as-is without modification.

### Media library upload workflow

Most JIRA tickets include an image attachment showing what the icon looks like in the Sitecore Pages editor. This image must be uploaded to the Sitecore media library before the URL is known. When this is the case, follow this workflow:

1. Generate the full SPE script with `$icon = ""` as a placeholder.
2. After all files are created, ask the developer to upload the icon image from the ticket to the Sitecore media library.
3. Ask the developer to provide the resulting media library URL (in the format `/-/media/<guid>.ashx?thn=1&&db=master`).
4. Once the developer provides the URL, update the `$icon` variable in the generated SPE script to the provided value.
...
### Example : media library icon (after upload)

```powershell
$icon = "/-/media/fb3d1e61682a4dd09472092233201c9d.ashx?thn=1&&db=master"
```

### Example : icon placeholder pending upload

```powershell
$icon = "" # TODO: replace with media library URL after uploading the icon image from the ticket
```

### Example : icon that's undefined in the ticket
...
### `$vxaDatasourceBranchTemplatePath` : GLOBAL VARIABLE

This variable specifies the Sitecore path to the branch templates folder. The default value for this project is `/sitecore/templates/Branches/Feature/Velir Experience Accelerator`. Before generating the script, ask the developer:

> The default branch template path is `/sitecore/templates/Branches/Feature/Velir Experience Accelerator`. Would you like to use this default, or override it with a different path?

Use the developer's confirmed value when setting `$vxaDatasourceBranchTemplatePath` in the Variables section of the generated script.
